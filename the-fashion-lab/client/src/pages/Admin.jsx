...
const empty={name:"",description:"",category:"Vintage",gender:"Unisex",size:"M",condition:"Excellent",price:"",old_price:"",image:"",stock:1};
export default function Admin({user}){const [products,setProducts]=useState([]),[orders,setOrders]=useState([]),[f,setF]=useState(empty),[editing,setEditing]=useState(null),[err,setErr]=useState(""),[busy,setBusy]=useState(false);
 ...
 if(user?.role!=="admin")return <main className="page"><h1>Admin access required.</h1><p className="muted">Sign in with the administrator account to manage the store.</p></main>;
 const change=(k,v)=>setF(x=>({...x,[k]:v}));
 async function save(e){e.preventDefault();setBusy(true);setErr("");try{const body={...f,price:Number(f.price),old_price:f.old_price?Number(f.old_price):null,stock:Number(f.stock)};if(editing)await api(`/products/${editing}`,{method:"PATCH",body:JSON.stringify(body)});else await api("/products",{method:"POST",body:JSON.stringify(body)});setF(empty);setEditing(null);await load()}catch(e){setErr(e.message)}finally{setBusy(false)}}
