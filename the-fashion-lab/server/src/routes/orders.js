...
router.post('/create',auth,async(req,res)=>{
  ...
  try{
    ...
    const order=(await client.query(`INSERT INTO orders(user_id,total,shipping_name,shipping_phone,shipping_address,payment_method)
      VALUES($1,$2,$3,$4,$5,$6) RETURNING *`,
      [req.user.id,total,shipping.name||'',shipping.phone||'',shipping.address||'',payment_method])).rows[0];
    for(const item of items){
      const p=byId[item.productId];
